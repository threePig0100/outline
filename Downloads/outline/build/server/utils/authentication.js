...
async function signIn(ctx, service, _ref) {
  ...
  if (team.subdomain) {
    ...
    if (client === _types.Client.Desktop) {
      ctx.redirect("".concat(team.url, "/desktop-redirect?token=").concat(user.getTransferToken()));
    } else {
      let url = team.url.replace(":3002", ":3000");
      ctx.redirect("".concat(url, "/auth/redirect?token=").concat(user.getTransferToken()));
    }
  } else {
    ...
    ctx.redirect(!hasViewedDocuments && collection ? "".concat(team.url).concat(collection.url) : "".concat(team.url, "/home"));
  }
}